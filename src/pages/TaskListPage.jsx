...
export default function TaskListPage() {
  ...
  useEffect(() => {
    if (isError && allError) {
      toast.error(
        allError instanceof Error ? allError.message : typeof allError === 'string' ? allError : 'Something went wrong'
      )
    }
  }, [isError, allError])
  ...
  return (
    <div className="px-4 py-4 sm:px-6 lg:px-8">
      ...
      {isError && (
        <div className="flex items-center justify-center mt-10 text-center bg-white p-4 rounded-xl shadow-sm h-40">
          <p className="text-gray-400 text-lg font-bold">Sorry, Something went wrong</p>
          <button className="primaryBtn ml-4" onClick={() => refetch()}>
            Refetch
          </button>
        </div>
      )}
      ...
      {!isLoading && !isError && Array.isArray(tasks) && tasks.length === 0 && (
        <div className="flex items-center justify-center mt-10 text-center bg-white p-4 rounded-xl shadow-sm h-40">
          <p className="text-gray-400 text-lg font-bold">
            Sorry, No tasks were found, create a task so it can appear here.
          </p>
        </div>
      )}
